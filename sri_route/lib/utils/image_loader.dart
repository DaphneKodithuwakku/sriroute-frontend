...
class ImagePreloader {
  ...
  static Stream<ImageLoadingEvent> get loadingEvents =>
      _loadingEventController.stream;
  ...
  static Future<void> _tryLoadImage(
    ...
    if (_retryCount[url]! >= _maxRetries) {
      ...
      completer.completeError(
        'Failed to load image after $_maxRetries attempts',
      );
      _loadingEventController.add(
        ImageLoadingEvent(
          url: url,
    ...
    try {
      final cacheManager =
          isPanorama ? await panoramaCacheManager : _cacheManager;
  ...
  static Future<void> prioritizeImages(List<String> urls) async {
    ...
    for (var i = 1; i < notLoadedUrls.length; i++) {
      preloadImage(
        notLoadedUrls[i],
      ).catchError((e) => debugPrint('Background load error: $e'));
    }
  }
}
