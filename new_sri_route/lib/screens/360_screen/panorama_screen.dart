...
class _PanoramaScreenState extends State<PanoramaScreen>
  ...
  void _startBackgroundPreloading() {
    ...
    _preloadTimer = Timer.periodic(_preloadingTimerDuration, (_) async {
      try {
        ...
        final currentIndex =
            points.indexWhere((p) => p.id == _currentPoint!.id);
  ...
  Future<void> _preloadNextImagesInBackground(
      List<TourPoint> points, int currentIndex) async {
    try {
  ...
  Future<T> _timeoutFuture<T>(
      Future<T> future, Duration timeout, String timeoutMessage) {
    ...
    return Scaffold(
      body: FutureBuilder<List<TourPoint>>(
        ...
        builder: (context, snapshot) {
          ...
          final currentIndex =
              tourPoints.indexWhere((p) => p.id == currentPoint.id);
          ...
          return PopScope(
            ...
            child: Stack(
              children: [
                ...
                _buildUIOverlay(
                    context, tourPoints, currentPoint, currentIndex),
  ...
  Widget _buildPanoramaView(
      List<TourPoint> tourPoints, TourPoint currentPoint) {
  ...
  List<Hotspot> _buildHotspots(
      TourPoint currentPoint, List<TourPoint> tourPoints) {
    return currentPoint.hotspots
        .map((hotspot) => Hotspot(
              ...
              widget: PanoramaHotspotWidget(
                ...
                onTap: () {
                  final target =
                      _findPointById(tourPoints, hotspot.targetPointId);
                  if (target != null) {
  ...
  Widget _buildUIOverlay(BuildContext context, List<TourPoint> tourPoints,
      TourPoint currentPoint, int currentIndex) {
    return SafeArea(
      child: Column(
        children: [
          ...
          Padding(
            padding: EdgeInsets.symmetric(
                horizontal: _screenWidth * 0.05, vertical: 16),
  ...
  Widget _buildNavigationControls(
      List<TourPoint> tourPoints, int currentIndex) {
    ...
    return Row(
      ...
      children: [
        ...
        if (currentIndex > 0)
          ...
          const SizedBox(
              width: 50), // Smaller placeholder since we removed text
