...
""";
      ...
      return "There's a compatibility issue with Google Sign-In. Please try email login or update the app.";
        ...
        errorMessage
            .contains("is not a subtype of type 'PigeonUserDetails?'")) {
      return "There's a compatibility issue with Google Sign-In. Please try email login or update the app.";
  ...
  static Future<void> printDebugCertificateInfo() async {
    try {
      ...
      debugPrint(
          "To fix Error 10, add your SHA-1 fingerprint to Firebase Console:");
      debugPrint("1. Run this command to get your SHA-1:");
      debugPrint(
          "   keytool -list -v -keystore ~/.android/debug.keystore -alias androiddebugkey -storepass android -keypass android");
      debugPrint("2. The SHA-1 fingerprint will look like: AA:BB:CC:DD:...");
      debugPrint(
          "3. Add this to Firebase Console under Project Settings → Your Android app → Add fingerprint");
      debugPrint("-------------------------------------------");
    } catch (e) {
  ...
  static Future<UserCredential?> signInWithGoogle() async {
    try {
      ...
      try {
        ...
        if (e.toString().contains("type 'List<Object?>") &&
            e
                ...
                .contains("is not a subtype of type 'PigeonUserDetails?'")) {
          debugPrint(
              "Caught PigeonUserDetails type cast error. This is likely a version mismatch issue.");
          throw FirebaseAuthException(
              code: 'google-signin-cast-error',
              message:
                  'Google Sign-In package version mismatch. Please update the app.');
        }
        rethrow;
      }
    } catch (e) {
      ...
      if (FirebaseAuth.instance.currentUser != null) {
        debugPrint(
            "User is already authenticated despite the error. Continuing...");
      ...
      try {
        ...
        if ((e.toString().contains("type 'List<Object?>") &&
                (e.toString().contains(
                    ...
                    e.toString().contains(
                        "is not a subtype of type 'PigeonUserInfo'"))) ||
